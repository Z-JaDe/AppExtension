...
public protocol NSManagedObjectProtocol: class, NSFetchRequestResult {
...
extension EntityManager {
    // MARK: - 增
    /// ZJaDe: 创建model
    public func create<T>(_ closure: (T) -> Void) -> T where T: NSManagedObject & NSManagedObjectProtocol {
    ...
    public func createIfNeed<T>(keyValues: [(String, Any)], _ closure: (T) -> Void) -> T where T: NSManagedObject & NSManagedObjectProtocol {
        let model: T
    ...
    // MARK: - 删
    /// ZJaDe: 删除所有相关model
    public func delete<T>(_ objects: [T]) where T: NSManagedObject & NSManagedObjectProtocol {
        let contexts = self.managedContext()
    ...
    public func delete<T>(keyValues: [(String, Any)]) -> [T] where T: NSManagedObject & NSManagedObjectProtocol {
        let contexts = self.managedContext()
    ...
    // MARK: - 改
    /// ZJaDe: 更新所有相关model
    public func update<T>(keyValues: [(String, Any)], _ closure: (T) -> Void) -> [T] where T: NSManagedObject & NSManagedObjectProtocol {
    ...
    public func updateFirst<T>(keyValues: [(String, Any)], _ closure: (T) -> Void) -> T? where T: NSManagedObject & NSManagedObjectProtocol {
        let model: T
        if let result: T = searchFirst(keyValues: keyValues) {
            model = result
            keyValues.forEach({ (keyValue) in
    ...
    public func updateFirstAndCreateIfNeed<T>(keyValues: [(String, Any)], _ closure: (T) -> Void) -> T where T: NSManagedObject & NSManagedObjectProtocol {
        let model: T
    ...
    public func search<T>(predicate: NSPredicate? = nil) -> [T] where T: NSManagedObject & NSManagedObjectProtocol {
        let contexts = self.managedContext()
        let fetchRequest: NSFetchRequest<T> = T.fetchRequest()
    ...
    public func search<T>(keyValues: [(String, Any)]) -> [T] where T: NSManagedObject & NSManagedObjectProtocol {
        let objects: [T]
    ...
    public func searchFirst<T>(keyValues: [(String, Any)]) -> T? where T: NSManagedObject & NSManagedObjectProtocol {
