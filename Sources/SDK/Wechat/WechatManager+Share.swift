...
extension WechatManager: ShareItemProtocol {
    // MARK: - 分享
    public func shareToWeChat(_ shareModel: ShareModel, _ callback:@escaping CallbackType) {
        ...
        if result == false {
            self.shareCallBack(isSuccessful: false)
        }
    }
    public func shareToWeChatTimeline(_ shareModel: ShareModel, _ callback:@escaping CallbackType) {
        self.setShareCallback(callback)
        guard WechatManager.canUseWeChat() else {
...
extension WechatManager {
    ...
    func onShareResp(_ resp: SendMessageToWXResp) {
        ...
        case WXSuccess:
            ...
            self.shareCallBack(isSuccessful: true)
