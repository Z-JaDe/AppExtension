...
@IBDesignable open class ScrollingPageControl: UIView {
    ...
    fileprivate lazy var ringLayer: CALayer = {
        let layer = CALayer()
    ...
    fileprivate func center(_ layer: CALayer) {
        let frame = CGRect(
            x: (self.bounds.size.width - layer.bounds.size.width)/2,
            y: (self.bounds.size.height - layer.bounds.size.height)/2,
            width: layer.bounds.size.width,
            height: layer.bounds.size.width)
        layer.frame = frame
    }
    ...
    fileprivate func layoutPageIndicators(_ layers: [CALayer], container: CALayer) {
        ...
        container.frame = CGRect(x: 0, y: 0, width: layerFrame.origin.x, height: ringLayer.bounds.size.height)
    }
